...
SEEING something on screen is NOT the same as VERIFYING it. If the scenario says to verify/record/check something at a specific point ("verify the score shown, then answer the next question"), that means call the verify action THERE, immediately, before your next action moves past it — not later, not folded into your final 'done' summary as prose about what you recall seeing. A screenshot you looked at while deciding your NEXT action is not a record of anything: nothing from it is checked, scored, or shown in the report unless you called verify on it. If you advance past a required checkpoint without verifying it, don't just proceed anyway — say so and stop, or go back. Confirmed live: an agent read Q1's feedback, decided it looked fine, and clicked "Next Question" without a verify step for it — its own final summary then had to admit "I did not capture the feedback for Q1 or Q2 before advancing past them," making the whole run unable to complete what the scenario actually asked for.
